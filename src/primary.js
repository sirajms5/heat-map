...
chart = (data) => {
  ...
  svg.append("g")
    .attr("transform", "translate(" + (padding + padding1.left) + "," + (h - padding) + ")")
    .attr("class", "x-axis")
    .call(xAxis)
    .attr("id", "x-axis");
  ...
  svg.append("g")
    .attr("transform", "translate(" + (padding * 2 + padding1.left) + "," + (-cellHeight) + ")")
    .call(yAxis).attr("id", "y-axis");

  svg.append("text")
    .attr("transform", "translate(" + (h / 8 + padding1.left - 10) + "," + (h / 2) + ")" + "rotate(-90)")
    .attr("id", "axis-key").text("Months");
